...
(function() {
	...
	var QRCreator = {
		...
		addIcon: function(isAlert) {
			...
			<overlay xmlns="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul" \
				...
				<toolbarpalette id="mainPopupSet">\
					...
					<menuitem label="自定文字" tooltiptext="输入文字创建二维码" oncommand="QRCreator.launch(2);"/>\
						...
						<menuseparator/>\
				 		<menu label="图标位置">\
							<menupopup id="QRCreator_Icon_Pos">\
			...
			#QRCreator-panel > hbox:first-of-type > label.QRCreator_panel_QRImage_label {\
				font-size: 18px;\
				color: #06c;\
			}\
			#QRCreator-panel #QRCreator_panel_loadingImage {\
				max-height: 18px;\
				max-width: 18px;\
			}\
			#QRCreator_panel_QRImage {\
				min-height:150px;\
				min-width:150px;\
				max-height:200px;\
				max-width:200px;\
		...
		showImage: function(src, target_data, altText) {
			...
			label  = $C("label", {
				class: "QRCreator_panel_QRImage_label",
